...
def callback(request):
    ...
    # 1.这是微信给我们返回的code和state
    code = request.GET.get("code")
    # state是我们第一次扫码带过去的session，存的是用户UID
    state = request.GET.get("state")

    # 2.然后需要我们再发一次请求回去(相当于握手！)
...
def sendmsg(request):
    ...
    openid = models.UserInfo.objects.get(id=user_id).wx_id
